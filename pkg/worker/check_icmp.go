...
func newICMPChecker(c SyntheticsModelCustom) *icmpChecker {
	return &icmpChecker{
		c: c,
		details: map[string]float64{
			"packets_sent":     0,
...
/*func CheckPingRequest(c SyntheticsModelCustom) {
	...
	if err != nil {
		...
		if !isTestReq {
			...
			details["packets_sent"] = float64(stats.PacketsSent)
			details["packets_received"] = float64(stats.PacketsRecv)
			details["packet_loss"] = stats.PacketLoss
			details["latency_min"] = timeInMs(stats.MinRtt)
			details["latency_max"] = timeInMs(stats.MaxRtt)
			details["latency_avg"] = timeInMs(stats.AvgRtt)
			details["latency_std_dev"] = timeInMs(stats.StdDevRtt)
			...
			testBody["rcmp_status"] = "SUCCESSFUL"
			testBody["packet"] = fmt.Sprintf("%d packets sent, %d packets received, %f%% packet loss", stats.PacketsSent, stats.PacketsRecv, stats.PacketLoss)
			testBody["latency"] = fmt.Sprintf("min/avg/max/stddev = %f/%f/%f/%f ms",
				timeInMs(stats.MinRtt),
				timeInMs(stats.AvgRtt),
				timeInMs(stats.MaxRtt),
				timeInMs(stats.StdDevRtt))
			// packet_size in bytes
			testBody["packet_size"] = pinger.Size
		}
	}
